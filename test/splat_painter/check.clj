...
(defn -main [& _]
  ...
  (let [{:keys [vs-src gs-src]} (gen/sources)]
    ...
    (assert-contains gs-src "float tcap = (ssz2 < 3.5) ? 1.0 : (ssz2 < 8.0) ? 0.7 : 0.35;" "progressive colour-specificity ceiling")
    ...
    (assert-contains gs-src "float lal = (ssz2 >= 8.0) ? 1.0 : (ssz2 >= 2.5) ? 0.85 : 0.95;" "per-physical-size glaze alpha")
    (assert-contains gs-src "float traw = (ssz2 < 1.5) ? 0.85 : (ssz2 < 3.5) ? 0.7 : (ssz2 < 8.0) ? 0.45 : 0.0;" "per-physical-size raw floor")
