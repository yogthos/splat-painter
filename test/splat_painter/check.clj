...
(defn -main [& _]
  ...
  (let [{:keys [vs-src gs-src]} (gen/sources)]
    ...
    (assert-contains gs-src "else if (dmx > (liner ? 0.3 : 0.22)) fade *= 0.4;" "stroke fades at colour drift")
    ...
    (assert-contains gs-src "float mv = mapAt(u_sharp[k], px, py) * gain;" "line-hold sharp-map sample")
