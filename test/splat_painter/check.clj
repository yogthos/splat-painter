...
(defn -main [& _]
  ...
  (let [{:keys [vs-src gs-src]} (gen/sources)]
    ...
    (assert-contains gs-src "if (dmx > ((lvl <= 1) ? 0.18 : 0.45)) fade = 0.0;" "brush lifts at colour-region exit (broad lifts immediately)")
    ...
    (assert-contains gs-src "if (snapE) { vec2 sp3 = edgeSnap(px, py, (lvl >= 4) ? 0.35 : 0.65); px = sp3.x; py = sp3.y; }" "per-step ridge correction (gentle on liners)")
    ...
    (assert-contains gs-src "float mloc  = 1.0 + (u_broad - 1.0) * (1.0 - sabs);" "bokeh-adaptive broad multiplier on ABSOLUTE subjectness")
    (assert-contains gs-src "float melt = (lvl <= 1) ? clamp((u_broad - 1.0) / 1.5, 0.0, 1.0) * (1.0 - sabs) : 0.0;" "broad-tier bokeh melt")
    (assert-contains gs-src "float bgate = 1.0 - clamp((u_broad - 1.0) / 1.5, 0.0, 1.0) * (1.0 - min(1.0, sabs / 0.35));" "Broad-tied mid/fine bokeh gate")
    (assert-contains gs-src "float wsl = (lvl >= 4) ? 0.35 * tt : ((melt > 0.0) ? 0.85 * melt * tt : 0.0);" "canvas re-mix incl. melted broad chains")
    ...
    (assert-contains gs-src "float mv = mapAt(2, px, py) * (0.25 + 0.75 * sgate);" "line-hold sharp-map sample")
    (assert-contains gs-src "if (mv < 0.35 * th) fade = 0.0;" "line-hold brush lift")
    ;; liner tier keeps only a hint of the head taper (chains hand off into one rod)
    (assert-contains gs-src "float hw = (lvl >= 4) ? 0.8  + 0.2  * smoothstep(0.0, 0.18, tt)" "liner-muted head width taper")
    (assert-contains gs-src "float ha = (lvl >= 4) ? 0.75 + 0.25 * smoothstep(0.0, 0.15, tt)" "liner-muted head alpha taper")
