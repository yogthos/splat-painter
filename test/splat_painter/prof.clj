...
(defn -main [& [path szs]]
  (let [path (or path "DrawingWithGaussians/inputs/eye.jpeg")
        ...
        [blurh bh-ms]  (once-ms #(structure/dominant-blur img0 (max 6 (quot (:height img0) 80))))
        img (assoc img0 :structure sfield :detail dmap :blur blur :blur-heavy blurh :noise-fields (seed/prep-noise sfield))
        _ (println (format "ONE-TIME load: analyze %.0f  detail %.0f  blur %.0f  coverage-colour %.0f ms"
                           an-ms d-ms b-ms bh-ms))
        H  (double (:height img))
        ;; placement-map stats
